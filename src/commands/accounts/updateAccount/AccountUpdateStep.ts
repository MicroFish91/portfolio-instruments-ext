...
import { UpdateAccountApiResponse, updateAccount } from "../../../sdk/accounts/updateAccount";
...
export class AccountUpdateStep<T extends AccountUpdateContext> {
    ...
    async execute(context: T, progress: Progress<{ message?: string; increment?: number }>) {
        ...
        const response: UpdateAccountApiResponse = await updateAccount(context.token, context.account.account_id, {
